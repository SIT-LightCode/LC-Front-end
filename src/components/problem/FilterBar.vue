...
const filterValue = ref({ keyword: "", tag: "", level: 0, isOfficial: "" ,Submitted:""})
const resetFilter = () => {
        emit('filterValue', { keyword: "", tag: "", level: 0, isOfficial: "",Submitted:"" });
        filterValue.value.keyword = "";
        filterValue.value.tag = "";
        filterValue.value.level = 0;
        filterValue.value.isOfficial = "";
        filterValue.value.Submitted = "";
...
<template>
        <div class="mt-10 grid grid-cols-3 gap-3 ">
                ...
                <div class="flex items-center gap-1">
                        <p class="text-sm text-st-blue">Submitted:</p>
                        <select @change="$emit('filterValue', filterValue);" v-model="filterValue.Submitted"
                                class="border-none rounded-3xl px-4 py-1 w-36 text-sm	">
