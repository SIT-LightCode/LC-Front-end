...
<template>
    <div class="border-2 rounded-lg p-5 m-5">
        <div>
            ...
            <h2>
                Level : <span v-for="level in prop.data.level">
                    <StarOutlined />
                </span>
            </h2>
            <h2>Tag : <span v-for="t in data.tagProblem"
                    class="inline-flex items-center px-3 rounded-full text-xs font-medium leading-4  text-gray-800"
                    :class="colorTags[(t.tag.id - 1) % 8]">{{ t.tag.topic }} </span></h2>
        </div>
        <div>
            <h1>Description</h1>
        ...
        <div>
            <div v-for="(example, index) in  prop.data.example" class="border-2 rounded-lg p-5 items-center m-5">
                <div >
                ...
                <div >
                    <span class="p-5 m-5">Output:</span> <span class="p-5"> {{ example.output }}</span>
                </div>
            </div>
        </div>
        <ResultPage v-if="prop.result != {}" :data="prop.result"  class="border-2 rounded-lg p-5 m-5"></ResultPage>
    </div>
</template>
