...
<template>
  <div class="mb-2 text-sm font-medium text-gray-900 dark:text-white">
    ...
    <div v-for="(i, key1) of example.example.val" class="py-5">
      ...
      <div class="flex flex-row">
        <div v-for="(j, key2) of i" class="flex flex-row py-3">
          ...
          <input
            rows="4"
            v-bind:class="csstextarea"
            v-model="example.example.val[key1][key2]"
          />
        </div>
        <div class="flex flex-col">
          <button
            :disabled="example.example.val[key1].length > 3"
            class="text-gray-900 dark:text-white rounded-full flex-auto w-25 p-3 mx-5 my-1"
            :class="[
              example.example.val[key1].length > 3
                ? 'bg-gray-300 '
          ...
          <button
            :disabled="example.example.val[key1].length <= 1"
            class="text-gray-900 dark:text-white rounded-full flex-auto w-25 p-3 mx-5 my-1"
            :class="[
              example.example.val[key1].length <= 1
                ? 'bg-gray-300 '
                : 'bg-blue-400 hover:bg-blue-500',
            ]"
    ...
    <button
      :disabled="example.example.val.length > 3"
    ...
    <button
      :disabled="example.example.val.length <= 1"
      class="text-gray-900 dark:text-white font-bold m-2 py-2 px-4 rounded-full"
      :class="[
